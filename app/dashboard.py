...
def direct_model_options(provider: str) -> list[str]:
    defaults = DIRECT_PROVIDER_MODELS.get(
        provider, [DIRECT_DEFAULT_MODELS.get(provider, "")]
    )
...
def current_llm_config() -> dict:
    ...
    model_name = st.session_state.get("direct_model") or DIRECT_DEFAULT_MODELS.get(
        provider, ""
    )
    return {
        "route_mode": "direct",
...
def api_request(url: str, data: dict, files, spinner_text: str):
    with st.status(spinner_text, expanded=True) as status:
        ...
        if response.status_code == 200:
            ...
            status.update(
                label=f"Request failed ({response.status_code})", state="error"
            )
    return response
...
def generate_txt_export(result_data: dict) -> bytes:
    ...
    for index, suggestion in enumerate(
        result_data.get("improvement_suggestions", []), 1
    ):
...
def _render_sidebar():
    with st.sidebar:
        ...
        if selected_route == "experiential":
            ...
            selected_row = next(
                (r for r in catalog if r["model"] == selected_model), {}
            )
            ...
            if status == "PROMOTION":
                ...
                st.markdown(
                    '<span class="model-free">FREE</span>', unsafe_allow_html=True
                )
            else:
                st.markdown(
                    '<span class="model-paid">PAID</span>', unsafe_allow_html=True
                )
            st.caption("Lane: Experiential Cloud")
        else:
            provider_options = [
                p for p in PROVIDER_LABELS.keys() if p != "experiential"
            ]
            ...
            if not catalog:
                ...
                for row in catalog:
                    ...
                    if row.get("free"):
                        ...
                        pricing_line = (
                            f"${in_per_k:.4f} in / ${out_per_k:.4f} out per 1K"
                        )
                    st.markdown(
        ...
        if st.session_state.get("last_analysis") and st.button(
...
with st.container(border=True):
    ...
    quota_snapshot = (
        fetch_quota_status(st.session_state.get("api_base", DEFAULT_API_BASE)) or {}
    )
    active_quota = quota_snapshot.get(cfg["provider"], {})
    active_minute = (
        (active_quota.get("windows", {}) or {}).get("minute", {})
        if active_quota
        else {}
    )
    rpm_limit = (
        (active_quota.get("limits", {}) or {}).get("rpm", 0) if active_quota else 0
    )
    rpm_used = active_minute.get("used_requests", 0) if active_minute else 0
    rpm_pct = (
        float(active_minute.get("percent_used_requests", 0.0) or 0.0)
        if active_minute
        else 0.0
    )
    h3.markdown(
    ...
    status_text = (
        ...
        else "Approaching limit" if rpm_pct < 100 else "At limit"
    )
    status_class = (
        "health-ok"
        if not rpm_limit or rpm_pct < 85
        else "health-warn" if rpm_pct < 100 else "health-bad"
    )
    h4.markdown(
                ...
                for label, window, key in [
                    ...
                    if limits.get(key, 0):
                        ...
                        color = (
                            "#10b981"
                            if pct < 60
                            else "#f59e0b" if pct < 85 else "#ef4444"
                        )
                        st.markdown(
...
def _render_log_panel():
    with st.expander("🛠️ Backend processing log", expanded=False):
        ...
        if logs:
            ...
            st.caption(
                f"Backend log file: {log_file_path} · "
                f"{len(filtered)}/{len(logs)} events shown"
            )
        else:
            st.info(
...
def _render_cv_generation():
    ...
    if _actionable_preview:
        ...
        st.caption(
            "No actionable improvements were detected — universal ATS rules will apply."
        )
...
def _render_advanced_tools():
    ...
    with tool_tab1:
        orig_text = st.text_area(
            "Original Bullets (one per line)", height=100, key="diff_orig"
        )
        opt_text = st.text_area(
            "Optimized Bullets (one per line)", height=100, key="diff_opt"
        )
        if st.button("Compare Bullets", width="stretch", key="diff_compare_btn"):
            payload = {
                "original_bullets": [
                    x.strip() for x in orig_text.splitlines() if x.strip()
                ],
                "optimized_bullets": [
                    x.strip() for x in opt_text.splitlines() if x.strip()
                ],
            }
            try:
                response = requests.post(
        ...
        if st.button(
            ...
            if not bulk_job_desc.strip() or not bulk_files:
                ...
                if response and response.status_code == 200:
                    ...
                    st.success(
                        f"Successfully processed {len(results)} candidate resumes."
                    )
...
def _render_career_suite():
    ...
    with f_tab2:
        comp_name = st.text_input(
            "Company Name", value="Target Company", key="cover_company"
        )
        cov_tone = st.selectbox(
            "Tone", ["formal", "startup", "technical"], key="cover_tone"
        )
